...
//--------------------------------------------------------------------------------------------------------------------------
  ...
  if(mode == mode_calibrate) {
    Serial1.println("Mode calibrate");
    Serial1.println("Début de la calibrations");
    calibrate();
    Serial1.println("L'axe à été calibré");
  }
...
void readMode() {
  ...
  {
    Serial1.println("Steppeur ont été coupé apres 1 min d'inactivité");
    stopSteppeur();
    startReadMode = -2;
  }
}
  ...
  for (int i=0; i<9; i++)
  {
    ...
    switch (drinkToServ[i]) 
    {
      ...
      case 6:
        ...
        break;
      default:
        Serial1.println("Aucune Bouteille ne correspond à cette place");
    }
  }
...
void moveTo(int nextPos)
{
  ...
  if (stepToMove==0) 
  {
    Serial1.println("Déjà au bon endroit");
    pos = nextPos;
    hautBas();
    delay(1000);
    return;
